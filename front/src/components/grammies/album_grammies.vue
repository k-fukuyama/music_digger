<template>
  <v-app>
    <v-form @submit.prevent="createGrammyOfAlbumType">
      <v-container>
        <v-row align="center">
          <v-col cols="12" sm="6">
          <v-autocomplete
      ...
      <v-btn large color="primary" @click="createGrammyOfAlbumType">グラミー受賞作品に登録</v-btn>
    </v-form>
  ...
  export default {
    data: function() {
      return {
        ...
        artists: [],
        artist_albums: [],
        grammy_types: [{name: '最優秀アルバム賞', value: 1}],
        grammy_artist_id: '',
        grammy_album_id: '',
        grammy_type: '',
        grammy_year: '',
    ...
    methods: {
      ...
      if (!this.grammy_artist_id) return;
        ...
        axios.get('/api/v1/albums/search', {params: { artist_id: this.grammy_artist_id }}).then((res) => {
          this.artist_albums = res.data
        }, (error) => {
          console.log(error);
        });
      },
      createGrammyOfAlbumType: function () {
        if (!this.grammy_artist_id || !this.grammy_album_id || !this.grammy_type) return;

        axios.post('/api/v1/grammies', { artist_id: this.grammy_artist_id, album_id: this.grammy_album_id, grammy_type: this.grammy_type, genre_id: this.genre_id, year: this.grammy_year }).then((res) => {
